...
class Trie {
	...
	public:
		...
		void salve(string saida) {
			...
			if(n != lista.size()) {
				cerr << "Estado invalido: mais codigos presentes que o esperado na trie!" << endl;
				exit(1);
			}

			// Numero de bits por codigo
			int nbits_cod = get_nbits(n);
			...
			// Lista codigo,char
			escreve_int(out,lista.size(),32); // no maximo 2^31-1 = 2147483647 entradas na trie.
			for(int i = 0; i < lista.size(); i++) {
				escreve_int(out,lista[i].first,nbits_cod);
				escreve_int(out,lista[i].second,nbits_char);
			}
